...
impl Facts {
    ...
    pub fn ssh_answered(&mut self, vm: &vm::Vm) {
        ...
        // what else lima holds, so `vm_strays` is not touched here -- it
        // keeps whatever the survey found. An earlier version copied it
        // into the local `Survey` below to "carry it through"; nothing
        // read it there, so the copy said something the code did not
        // do.
...
pub fn kept(facts: &Facts, data: bool) -> Vec<String> {
    ...
    if facts.vm_base_exists {
        ...
        // this asks the narrower question of whether the stray is a VM
        // directory or protected disk *inside `[vm] dir`*, since those
        // are the kinds the sentence about `[vm] dir` has to carve out.
        ...
        keep.push(format!(
            ...
            // A `[vm] dir` nobody could read is called "safe to
            // remove" here, which is #192's to fix: this change reports
            // what it found and says nothing about what it could not
            // look at.
            if holds_a_vm_directory_or_protected_disk {
...
/// now that the unreadable-directory sentence has gone to #192, which
/// will want it back; it is kept because the style belongs to the
/// report rather than to `lima_removed`.
