...
async fn command_main(args: impl IntoIterator<Item = std::ffi::OsString>) -> Result<()> {
...
fn companion_client_path(server: &Path) -> PathBuf {
    let name = server
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or_default();
...
mod tests {
    ...
    fn release_asset_binaries_find_their_companions() {
        ...
        assert_eq!(
            vm::companion_server_path(Path::new("/tmp/ssf-0.4.0-linux-x86_64")),
