...
- The installed service runs the last package the maintainer installed, so verify daemon behaviour with unit tests and scratch `SSF_CONFIG_DIR`/`SSF_STATE_DIR` runs rather than expecting to see your change live."
        ...
        let catalogue: Vec<(&str, String)> = vec![
            ...
            (
                ...
                followup_prompt(&issue18, &[comment.clone()], &own),
            ),
            (
                "followup_prompt on an owned PR (review requested from the bot)",
                followup_prompt(&pr22, &[requested.clone()], &owned_pr),
            ),
            (
            ...
            (
                ...
                closed_prompt(&issue18, &[closed.clone()], &own),
            ),
            (
                "unassigned_prompt",
                unassigned_prompt(&issue18, &[unassigned.clone()], &own),
            ),
            (
                "reassigned_prompt",
                reassigned_prompt(&issue18, &[reassigned.clone()], &own),
            ),
            (
            ...
            (
                ...
                review_followup_prompt(&pr22, &[reply.clone()], &reviewer),
            ),
            (
                "review_again_prompt (repeated request)",
                review_again_prompt(&pr22, &[again.clone()], &reviewer),
            ),
            (
