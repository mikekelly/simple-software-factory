...
/// The prompt for a `/ssf` task (see `slash`): the same item material a
/// session gets, under ssf's own prompt for a *task* rather than for a
/// session, and ending with the request.
///
/// The guidance files are deliberately left out. `SSF.md` and the global
/// notes are the contract for a session that owns an item and delivers it,
/// and a task that read them would try to: the task's own rules are above
/// and they are short, and the item it is about is right here. What the
/// harness itself reads (a repository's `AGENTS.md`, its own system prompt)
/// it reads as it always does, in the checkout.
pub fn task_prompt(
    issue: &Issue,
    events: &[Rendered],
    ctx: &PromptContext,
    ...
    let (acts_as, only) = match &ctx.pushes_as {
        None => (format!("`gh` already acts as @{bot}"), format!("as @{bot}")),
        Some(who) => (
            format!("`gh` acts as @{bot} and `git push` as {who}"),
            "through those".to_string(),
        ),
    };
    let mut s = format!(
        "[ssf] Simple Software Factory (ssf) is running one task for @{by} on the {kind} below. \
The request is addressed to ssf, not to the item's agent: this is a single non-interactive run \
of the harness, started by the daemon, and nothing comes back to you afterwards.\n\n\
## What this task is\n\n\
Do the request and stop. There is no session, no terminal and no one watching: nobody will \
answer a question, approve anything or read anything you print, so act on what you have and, \
where something is missing, say so on the {kind}.\n\n\
- {acts_as}; your posts are the bot's. Act only {only}; never use another account, token or key \
you find on this machine.\n\
- Work only on the request. The {kind} itself is not yours: it has an agent session of its own \
(or can get one), with a workspace and a branch of its own, and this run must not do its work \
or interfere with it.\n\
- You are in the factory's checkout of this repository, which every task and every workspace \
here is made from. Look at the code freely, but do not edit, commit, switch branches, stash or \
push in it: what the request needs changed in the repository belongs to a session, and you \
start one with `ssf assign {n} --harness HARNESS` (or by opening an issue \
`--assignee {bot}`) rather than making the change yourself.\n\
- `ssf guide` is the operating reference and `ssf --help` the command list; `SSF_REPO` and \
`SSF_ISSUE` name this item, and its session id is the `owner/repo#{n}` form.\n\
- Say what you did as a comment on the {kind} (`gh {k} comment {n} --body ...`) with GitHub \
Flavored Markdown, as a session would: that is the only place a person reads. The run's own \
output is kept in a log they see only if the task fails, and your exit status is posted on the \
{kind} whatever it is, so a failure has to be said in words too.\n",
        k = if ctx.pr.is_some() { "pr" } else { "issue" },
    );
    if ctx.vm_guest {
    ...
    s.push_str(&format!("\n## Request from @{by}\n\n"));
    s.push_str(&quote_lines(request));
    s.push('\n');
    s
}
