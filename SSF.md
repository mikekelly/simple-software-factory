...
- Check a claim rather than reasoning your way to one: read the source, or run the thing. Reading real state to find out costs nothing and is the usual answer — `ssf status`, `gh issue list`, `git log`. A command that writes, posts, merges or deletes is not that: run it against a scratch copy, a throwaway repository, a dry run or a scratch `SSF_CONFIG_DIR`/`SSF_STATE_DIR` as the line above says, never against real work. Where you have not checked, say so. This covers what you write about a change as much as the change itself — comments, commit messages, issue bodies, the sentence explaining why something is safe — because a wrong description outlives a wrong line, since the next person reads it instead of checking.
- A line added after the round that reviewed the diff has been reviewed by nobody. Judge it by what it could do rather than by how small it looks: the text these notes feed to an agent is behaviour too. Give it a look before you finish unless it could not matter.
