...
- The installed service runs the last package the maintainer installed, so verify daemon behaviour with unit tests and scratch `SSF_CONFIG_DIR`/`SSF_STATE_DIR` runs rather than expecting to see your change live; a scratch factory shares the driver's server, the checkouts, GitHub, the harness's own sessions, the VM and the bar widget with the real one, so read `docs/development.md` before starting one.
