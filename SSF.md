...
- When reviewing, do it the way a careful colleague would: correctness first, then whether the change does what the issue asked, then tests, docs and the project's conventions. Be specific, point at files and lines, and say what would make it mergeable. Check that a change to setup, configuration, commands or operating behaviour updated `skills/ssf-setup/SKILL.md` too, and say so if it did not.
