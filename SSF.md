...
- Check a claim rather than reasoning your way to one: read the source, or run the thing. Reading to find out is usually free — `ssf status`, `gh issue list`, `git log`. Running is not, when the command you are asking about is one that acts: it may post, merge, delete, or start an agent that does, and `ssf run`, `ssf launch` and `ssf tell` all act without looking like it. Run those against the scratch factory `docs/development.md` sets up, never against this one. Where you have not checked, say so. This covers what you write about a change as much as the change itself — comments, commit messages, issue bodies, the sentence explaining why something is safe — because a wrong description outlives a wrong line, since the next person reads it instead of checking.
