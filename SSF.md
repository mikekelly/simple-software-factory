...
- Check a claim rather than reasoning your way to one. How a tool parses its arguments, what a library does at boot, which path a command takes: run it, or read that source, and where you have not, say so. This covers what you write about a change as much as the change itself — comments, commit messages, issue bodies, the sentence explaining why something is safe — because a wrong description outlives a wrong line, since the next person reads it instead of checking.
- A line added after the round that reviewed the diff has been reviewed by nobody. Keep such a change trivially safe, or give it a look of its own.
