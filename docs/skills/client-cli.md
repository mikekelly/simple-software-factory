...
explains `peers`, `tell`, `sub`, `unsub`, `subs`, `handover` and `release` with
that session's context. `ssf skill sessions` provides the full lifecycle
reference. Release and purge protect worktrees holding unpushed work; resolve
that work before removal and do not bypass checks on someone's behalf.
