...
| `grok` | `GROK_AUTO_COMPACT_THRESHOLD_PERCENT`, set by `ssf launch` | Grok takes a percentage of the model's window, not a count. ssf divides the token count by the `context_window` Grok's own model cache (`$GROK_HOME/models_cache.json`, else `~/.grok/models_cache.json`) lists for the session's model, or the smallest window listed when no model is named, rounding down and keeping it within `1`-`100`: `300000` of a `500000` window is `60`. With no cache or no window for the model, the variable is not set and Grok compacts at its own threshold (`[session] auto_compact_threshold_percent`, default `85`). A value inherited from the operator's shell is cleared |
| `opencode` | `OPENCODE_CONFIG_CONTENT`, built by `ssf launch`: the launch model's `provider.<p>.models.<m>.limit` is set to `{context, input: <tokens>, output}`, with the real context and output taken from `opencode models <p> --verbose`, and `compaction.reserved` to `0` | OpenCode compacts once a message's tokens reach `limit.input - compaction.reserved`. It needs the launch model (`provider/model`); a session with none set, a model OpenCode does not list within 5 seconds, or a threshold at or over the model's listed `limit.input` or window (it already compacts sooner; a real limit is never raised) compacts at OpenCode's own point, with a notice. A configuration already in `OPENCODE_CONFIG_CONTENT` is kept: an `input` of its own for that model wins, and its `context` and `output` are kept, the listing filling only what it leaves out. A model switched to later in the session keeps OpenCode's default |
| everything else | nothing | `gemini`, `copilot` and `crush` have no such setting; a value configured above them is unused rather than an error, so one instance value can sit above a mixed set of repositories |
