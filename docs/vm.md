...
configured instance and external disk directories, their lexical ancestors
and resolved aliases, and Lima's current and legacy root and data-disk file
targets. A Firecracker `data.ext4` in one of their ancestors can receive a
file-only `rm -f` remedy; that ancestor never receives `rm -rf` over the
configured Lima storage.
