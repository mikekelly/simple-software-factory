...
| `vm.name`, `vm.dir` | `default`, `~/.local/share/ssf/vm` | The VM's name and where the image, kernel, binaries and each VM's files live (`<dir>/<name>/`); under lima the instance is `ssf-<name>` and its data disk `ssf-<name>` in lima's home, and `name` is then at most 7 characters, since lima labels the disk's filesystem `lima-<disk>` and an ext4 label holds 16. Changing `name` -- or `backend` -- renames nothing that exists, and what is left behind differs by backend. Under Firecracker the old `<dir>/<old name>/` stays, your clones in the `data.ext4` inside it. A bounded scan (32 levels, 4,096 entries) finds nested old names and stops below the first disk directory on each unrelated branch; directory symlinks and cutoff paths remain incomplete observations, while a non-configured link known to target a non-directory is irrelevant. A configured directory or data-disk link remains incomplete even when readable because destroy removes the link rather than its target; its resolved target and ancestors receive no recursive remedy. When an old directory is an ancestor or resolved alias of a nested new name, ssf reports the old `data.ext4` with an `rm -f` remedy and does not offer to remove its directory, which is part of the configured VM path. `[vm] dir` itself is the topmost protected ancestor, so a `data.ext4` directly in it is reported the same way and keeps the base from being called safe. If Lima's home is below `[vm] dir`, configured instance and disk directories, their lexical ancestors, and their resolved directory and known disk-file aliases receive the same protection. Under lima the instance `ssf-<old name>` and the disk `ssf-<old name>` stay in lima's home with the clones on the disk, while `<dir>/<old name>/` holds only lima's template, share and ssh key -- so nothing reports that directory, since a stray directory is one holding a `data.ext4`. `ssf uninstall` and `ssf vm status` name what they find as things ssf will not remove -- `ssf doctor` too, on a host that is not running the factory in a VM, since in VM mode it is forwarded into the guest, which cannot see the host's directories |
