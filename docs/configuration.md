...
| `auto_compaction_tokens` | `300000` | Context a session's harness may fill before it compacts its own history, in tokens, for every repository that does not set its own (`repo.auto_compaction_tokens`). Applied to the harnesses that take such a setting (`claude`, `codex`, `omp` and `grok`) and ignored by the rest, so one instance value can sit above a mixed set of repositories. `0` leaves each harness's own default alone (see [Context compaction](harnesses.md#context-compaction)) |
