...
| `dashboard.terminal_input` | `false` | Let the server web page's live terminal take control of an item's pane and type into it, where `item_pane_input` also allows it. Off, the page's terminals are read-only (see [the dashboard guide](dashboard.md#live-terminal-for-an-items-pane)); restart required |
...
| `repo.item_pane_input` | `daemon.item_pane_input` | Whether this repository's item panes take typing (`api/pane/input` and the live terminals) |
