...
| `pi` | `pi --approve -e "$SSF_PI_BRIDGE"` (Pi has no tool approvals; the flag trusts the repository's `.pi/` files; the extension handles safe item activity) | nothing (without `--approve`, a trust dialog ssf answers) |
| `omp` | `PI_STREAM_IDLE_TIMEOUT_MS=900000 omp --auto-approve -e "$SSF_PI_BRIDGE"` (the longer stream-idle window and delivery bridge are explained in [Workspaces and terminals](drivers.md)) | nothing |
...
`-e "$SSF_PI_BRIDGE"`; otherwise a live session has no safe item-activity
channel and `ssf doctor` asks for it to be restarted after the command is fixed.
