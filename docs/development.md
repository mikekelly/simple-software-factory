...
| `src/herdr/channel.rs`, `src/claude_delivery.rs`, `src/codex_delivery.rs`, `src/delivery_channel.rs` | the delivery `Channel` trait `Herdr::deliver` dispatches on, and its implementations: the terminal paste, Claude's peer inbox, Codex's app-server, and the OMP/Pi/OpenCode/Grok mailbox and bridges |
