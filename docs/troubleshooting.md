...
| An item has a `blocked` event saying the session is stuck at a harness question | the pane it names (`ssf vm attach` in a VM, herdr on the host) | the harness is at a screen ssf does not know, and a prompt typed into it would be lost, so ssf holds the prompt with no time limit. Answer the question in the pane; ssf sends the prompt once the pane is no longer blocked. For a first-run screen (theme, onboarding, trust, a new notice after an update), also re-run `ssf vm login <harness>` so later sessions do not meet it |
