...
//
// `render` is the Assign agent form, for an item with no agent. `renderActions`
// is the Actions row, for an item whose state is Working, Waiting on you, Done
// or Problem: Message, Hand over… (the assign pickers, prefilled with the stack
// the card is on, plus a note) and Release (a confirm step naming the branch).
...
`;
  ...
  /// The state for one item in one mode, most recent last. content.js draws one
  /// item at a time and rebuilds its nodes on every GitHub mutation, so this map
  /// is the only thing that outlives a page; a session keeps the items it has
  /// visited and forgets the rest. The item key carries the number as text, and
  /// the factory takes a number, so it is read here once.
  ...
  /// for it: no factory that watches this item accepts writes.
  ...
  /// or Problem, or `null` when no factory that watches it accepts writes.
  /// `item` is the card's own item object: the stack the hand-over pickers are
  /// prefilled from, and the branch the release confirm names.
  function renderActions({ factories, repo, number, item }) {
    const choices = contenders(factories);
    if (!choices.length) return null;
    const state = remember(`${repo}#${number}`, "actions");
